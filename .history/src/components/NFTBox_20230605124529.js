...
export default function NFTBox({ nftAddress, tokenId, price, seller, marketplaceAddress }) {
    ...
    const { data, runContractFunction: buyItem } = useWeb3Contract({
        abi: nftMarketplaceAbi,
        contractAddress: marketplaceAddress,
        functionName: "getListings",
        // msgValue: ethers.parseEther("0.2"),
        params: {
            nftAddress: nftAddress,
            tokenId: tokenId,
        },
    })
    /*  Since the buyItem function call isnt owrking-we will check with this function :
    getListings(address nftAddress, uint256 tokenId)
    
    The original code : functionName: "buyItem",
        msgValue: ethers.parseEther("0.2"),
        params: {
            nftAddress: nftAddress,
            tokenId: tokenId,
    ...
    */
    ...
    const handleBuyItemSuccess = async (data) => {
        console.log("getListings 's data before await", data)

        // await tx.wait(1)
        console.log("get listing bata return bhako price", data.price.toString())
        console.log("get listing bata return bhako seller", data.seller)

        dispatch({
            type: "success",
            message: "Item bought",
            title: " Buy successful.please refresh and (move blocks",
            position: "topR",
        })
        OnCloseModal()
    }
