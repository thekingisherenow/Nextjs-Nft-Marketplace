...
async function main() {
    ...
    const id = newStream1.toJSON().id
    const address = contractAddress
    // Now we attach address to the stream
    const result = await Moralis.Streams.addAddress({ address, id })
    console.log(`Steam created with streamId ${id} & contract Address  ${address} `)
    ...
    // const id = id1
    // // Now we attach address to the stream
    // const result1 = await Moralis.Streams.addAddress({ address, id })
